...
export function StepCep({ state, patch, busy, message, onCheck }: {
  state: OnboardingState; patch: Patch; busy: boolean; message: string; onCheck: () => void;
}) {
  return (
    <>
      ...
      <div className="onb__grid2">
        ...
        <Field id="onb-num" label="Número" value={state.numero} inputMode="numeric" placeholder="123"
          onChange={(v) => patch({ numero: v })} />
      </div>
      <Field id="onb-compl" label="Complemento (opcional)" value={state.complemento}
        placeholder="Apto, bloco, casa" onChange={(v) => patch({ complemento: v })} />
      {message && (
        <div className={`lp-wiz__msg ${state.disponivel === false ? "lp-wiz__msg--warn" : "lp-wiz__msg--ok"}`}>
          {message}
          {state.logradouro && (
      ...
      <button type="button" className="lp-btn lp-btn--outline lp-btn--lg" onClick={onCheck}
...
export function StepConta({ state, patch }: { state: OnboardingState; patch: Patch }) {
  return (
    <>
      ...
      <Field id="onb-nome" label="Nome completo" value={state.nome} autoComplete="name" onChange={(v) => patch({ nome: v })} />
      <div className="onb__grid2">
        <Field id="onb-email" label="E-mail" type="email" inputMode="email" autoComplete="email"
          value={state.email} onChange={(v) => patch({ email: v })} />
        <Field id="onb-tel" label="Celular" inputMode="tel" autoComplete="tel"
          value={state.telefone} onChange={(v) => patch({ telefone: maskPhoneBR(v) })} />
      </div>
...
export function StepTitular({ state, patch }: { state: OnboardingState; patch: Patch }) {
  return (
    <>
      ...
      <p className="onb__lead">É com eles que o contrato é emitido e a análise de crédito é feita.</p>
      <div className="onb__grid2">
        <Field id="onb-cpf" label="CPF" value={state.cpf} inputMode="numeric" placeholder="000.000.000-00"
          onChange={(v) => patch({ cpf: maskCPF(v) })} />
        <Field id="onb-nasc" label="Data de nascimento" type="date" value={state.nascimento}
          onChange={(v) => patch({ nascimento: v })} />
      </div>
      <p className="onb__micro">
        O CPF é usado só para a análise de crédito da contratação.
      </p>
    </>
  );
}

export function StepCredito({ state, busy, message, onRun }: {
  state: OnboardingState; busy: boolean; message: string; onRun: () => void;
}) {
  ...
  return (
    <>
      ...
      {state.credito ? (
        ...
        <button type="button" className="lp-btn lp-btn--cta lp-btn--lg" onClick={onRun} disabled={busy}>
  ...
  return (
    <>
      ...
      <div className="lp-wiz__doc">
        <ol>
          {contractClauses({ titular: state.nome, cpf: state.cpf, endereco, plano: state.plano }).map((c) => (
            <li key={c}>{c}</li>
          ))}
        </ol>
      </div>
      <label className="lp-wiz__check" htmlFor="onb-aceite">
        <input id="onb-aceite" type="checkbox" checked={state.aceite}
...
export function StepPagamento({ state, patch }: { state: OnboardingState; patch: Patch }) {
  return (
    <>
      ...
      <p className="onb__lead">
        Nada é cobrado agora. A primeira fatura sai depois da instalação, proporcional aos dias usados.
      </p>
      <div className="onb__opts">
        {PAGAMENTOS.map((p) => (
          <button key={p.key} type="button" className="onb__opt" aria-pressed={state.pagamento === p.key}
      ...
      <div className="onb__chips">
        {VENCIMENTOS.map((d) => (
          <button key={d} type="button" className="onb__chip" aria-pressed={state.vencimento === d}
...
export function StepInstalacao({ state, patch, minDate }: {
  state: OnboardingState; patch: Patch; minDate: string;
}) {
  return (
    <>
      ...
      <div className="onb__grid2">
        <Field id="onb-data" label="Dia" type="date" value={state.instalacaoData}
          onChange={(v) => patch({ instalacaoData: v })} />
        <div className="lp-field">
          <span style={{ display: "block", fontSize: "12.5px", fontWeight: 700, marginBottom: 6 }}>Turno</span>
          <div className="onb__chips">
            {TURNOS.map((t) => (
              <button key={t.key} type="button" className="onb__chip" aria-pressed={state.instalacaoTurno === t.key}
                onClick={() => patch({ instalacaoTurno: t.key as Turno })}>
                {t.label} <span style={{ fontWeight: 500, opacity: .8 }}>({t.desc})</span>
              </button>
            ))}
          </div>
        </div>
...
export function StepWifi({ state, patch }: { state: OnboardingState; patch: Patch }) {
  return (
    <>
      ...
      <Field id="onb-wifi-nome" label="Nome da rede" value={state.wifiNome} autoComplete="off"
        placeholder="Ex.: Casa da Ana" onChange={(v) => patch({ wifiNome: v })} />
      <Field id="onb-wifi-senha" label="Senha do Wi-Fi" value={state.wifiSenha} autoComplete="off"
